/* ----------------------------------------------------------------- 
FILE:	    23tree.c                                       
DESCRIPTION:This file contains the 23 tree routines for channel graph
	    generator.
CONTENTS:   tprint( v )
		TNODEPTR v ;
	    tdump( node )
		TNODEPTR node ;
	    tmin( root , node , value , property )
		TNODEPTR root , *node ; int *value , *property ;
	    tpop( root , node , value , property )
		TNODEPTR *root , *node ; int *value , *property ;
	    tmax( root , node , value , property )
		TNODEPTR *root , *node ; int *value , *property ;
	    ttrim( root , threshold )
		TNODEPTR *root ; int threshold ;
	    tdiscard( root , count )
		TNODEPTR *root ; int count ;
	    TNODEPTR tsearch( val , r )
		int val ; TNODEPTR r ;
	    taddson( root , r )
		TNODEPTR *root , r ;
	    tinsert( root , value , property )
		TNODEPTR *root ; int value , property ;
	    tpatch( v , value )
		TNODEPTR v ; int value ;
	    tdelete( root , value , property )
		TNODEPTR *root ; int value , property ;
	    tsubson( root , l )
		TNODEPTR *root , l ;
	    TNODEPTR tdsearch( value , r )
		int value ; TNODEPTR r ;
	    TNODEPTR tfind( value , r )
		int value ; TNODEPTR r ;
	    makenode( v , value , property ) 
		TNODEPTR *v ; int value , property ;
	    addplist( v , property ) 
		TNODEPTR v ; int property ;
	    PLISTPTR pfind( v , property , prevptr ) 
		TNODEPTR v ; int property ; PLISTPTR *prevptr ;
	    tprop( r , value )
		TNODEPTR r ; int value ;
	    PLISTPTR tplist( r , value )
		TNODEPTR r ; int value ;
DATE:	    Mar  3, 1990 
REVISIONS:  Mar  3, 1990 - Now a standalone program.
----------------------------------------------------------------- */
#ifndef lint
static char SccsId[] = "@(#) 23tree.c version 1.1 3/3/90" ;
#endif
...
qtop = 0 ;
qbot = 0 ;
q = (TNODEPTR *) Ysafe_malloc( 10001 * sizeof(TNODEPTR) ) ;
q[0] = v ;
while( qtop >= qbot ) {
    tdump( node = q[qbot++] ) ;
    if( node != v ) {
...
v = (TNODEPTR) Ysafe_malloc( sizeof( TNODE ) ) ;
v->son1 = r->son3 ;
v->son2 = r->son4 ;
...
if( r->father == (TNODEPTR) NULL ) {
    nr = (TNODEPTR) Ysafe_malloc( sizeof( TNODE ) ) ;
    nr->son1 = r ;
    nr->son2 = v ;
...
if( *root == (TNODEPTR) NULL ) {
	...
	if( plptr != (PLISTPTR) NULL ) {
		...
		if( f->nsons == 3 ) {
			...
			Ysafe_free( (*root)->son1 ) ;
			Ysafe_free( (*root)->son2 ) ;
...
(*v)->value = value ;
(*v)->nsons = 0 ;
(*v)->plist = (PLISTPTR) Ysafe_malloc( sizeof( PLIST ) ) ;
...
plptr = (PLISTPTR) Ysafe_malloc( sizeof( PLIST ) ) ;
plptr->pnext = v->plist ;
v->plist = plptr ;
v->plist->property = property ;
