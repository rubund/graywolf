...
void addCell( char *cellname, int celltype )
{
	...
	if( totalCellS >= cellAllocS ){
		cellAllocS += EXPECTEDNUMCELLS ;
		carrayG = (CBOXPTR *) Ysafe_realloc( carrayG, cellAllocS * sizeof( CBOXPTR ) ) ;
	}
	...
	if( celltype == STDCELLTYPE || celltype == EXCEPTTYPE ){
		numcellsG++ ;
	} else if( celltype == PADTYPE || celltype == PORTTYPE || celltype == PADGROUPTYPE || celltype == HARDCELLTYPE || celltype == SOFTCELLTYPE){
		ptrS->cclass = 0 ;
		ptrS->orflag = 0 ;
		numtermsG++ ;
...
void add_tile( int left, int bottom, int right, int top )
{
	...
	if( gate_arrayG ){
		if( curCellTypeS == STDCELLTYPE || curCellTypeS == EXCEPTTYPE || curCellTypeS == EXTRATYPE ) {
			if( width % spacer_widthG != 0 ) {
				sprintf( YmsgG, "the width of cell <%s> is not a multiple of the spacer_width\n", ptrS->cname ) ;
...
static void add_implicit_feed( char *pin_name, char *signal, int layer, int xpos, int ypos )
{
	ERRORABORT() ;
	...
	if( pin_layers_givenG ) {
		layer_test( layer ) ;
		imptrS->pinname = (char *) Ysafe_malloc((strlen( pin_name ) + 3 ) * sizeof( char ) ) ;
	...
	if( one_pin_feedthruG ) {
		if( pin_layers_givenG ) {
			imptrS->eqpinname = (char *) Ysafe_malloc((strlen(pin_name) + 3 ) * sizeof( char ) ) ;
...
static void check_pin( int xpos, int ypos, char *pinname )
{
	int layer ;
	char *find_layer() ;
	...
	tptr = ptrS->tileptr ;
	if( !turn_off_checksG ) {
		if( xpos < tptr->left || xpos > tptr->right || ypos < tptr->bottom || ypos > tptr->top ) {
			...
			sprintf( YmsgG, "\tcell l:%ld r:%ld b:%ld t:%ld  pin x:%d y:%d\n", tptr->left, tptr->right, tptr->bottom, tptr->top, xpos, ypos ) ;
			M( ERRMSG, NULL, YmsgG ) ;
			abortS = TRUE ;
		}
	}
	...
	if( pin_typeS != PASS_THRU ){
		...
		if(!(pinptrS->eqptr)){
			...
			if( ( ypos > 0 && eq_ypos < 0 ) || ( ypos < 0 && eq_ypos > 0 ) ) {
				...
				if( pin_layers_givenG != 0 ) {
					eqptrL->pinname = (char *) Ysafe_malloc((strlen(equiv_name) + 3 ) * sizeof( char ) ) ;
		...
		if( ( ypos > 0 && eq_ypos < 0 ) || ( ypos < 0 && eq_ypos > 0 ) ) {
			...
			if( ABS( eq_xpos - xpos ) < ABS( eqptrL->txoff ) ) {
				...
				eqptrL->typos = eq_ypos ;
				if( pin_layers_givenG ) {
					eqptrL->pinname = (char *) Ysafe_realloc( eqptrL->pinname, (strlen(equiv_name) + 3 ) * sizeof(char));
					sprintf( eqptrL->pinname , "]%1d%s", layer, equiv_name ) ;
				} else {
					Ysafe_realloc( eqptrL->pinname , (strlen(equiv_name) + 1 ) * sizeof(char));
		...
		if( pin_layers_givenG ) {
			imptrS->eqpinname = (char *) Ysafe_malloc((strlen( equiv_name ) + 3 ) * sizeof( char ) ) ;
...
static void layer_test( int layer )
{
	if( (layer < 0) || (layer > 4) ) {
...
void init_legal_blocks( int numblocks )
{
	ERRORABORT() ;
	ptrS->cclass = 0 ;
...
static int free_swap_data( int *data )
{
	Ysafe_free( data ) ;
} /* free_swap_data */

static void trans_tile( CBOXPTR ptr, int orient )
{
...
static void build_pad_group( int side, char *sidename, char *padgroupname )
{
	...
	if( build_padgroup ){
		...
		for( cell=numcellsG+1;cell<=lastpadG;cell++ ){
			ptr = carrayG[cell] ;
			if( ptr->padptr->padside == side && pptrS->lowerbound <= 0.0 && pptrS->upperbound >= 1.0 ){
...
void cleanup_readcells()
{
	...
	carrayG = (CBOXPTR *) Ysafe_realloc( carrayG, (totalCellS+1) * sizeof( CBOXPTR ) ) ;
	netarrayG = (DBOXPTR *) Ysafe_realloc( netarrayG, (numnetsG+1) * sizeof(DBOX) ) ;
...
void not_supported( char *object )
{
...
void add_corner( int x, int y )
{
	...
	if( pt >= ptAllocS ){
		ptAllocS += 2 ;
		pptrS->xpoints = (int *) Ysafe_realloc( pptrS->xpoints, ptAllocS * sizeof(int) ) ;
		pptrS->ypoints = (int *) Ysafe_realloc( pptrS->ypoints, ptAllocS * sizeof(int) ) ;
	}
