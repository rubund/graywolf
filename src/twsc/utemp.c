...
void from_middle()
{
	...
	sprintf(YmsgG,"%3d: %6.2le %6d%-8d%-6d%-6d", iterationG++,0.0,0,funccostG,rowpenalG,timingcostG);
	M( MSG, NULL, YmsgG ) ;
	sprintf(YmsgG,"%6d%4.2lf %4.2lf %5.2lf %4.2lf %4.2lf %4.2lf %5.3lf\n", P_limitG,0.0,binpenConG,roLenConG,timeFactorG,0.0,0.0,ratioG);
	M( MSG, NULL, YmsgG ) ;
	fflush(fpoG);
}
...
void from_beginning()
{
	...
	sprintf(YmsgG,"%3d: %6.2le %6d%-8d%-6d%-8d", iterationG++,0.0,0,funccostG,rowpenalG,timingcostG );
	M( MSG, NULL, YmsgG ) ;
	sprintf(YmsgG,"%6d %4.2lf %4.2lf %5.2lf %5.2lf %4.2lf %4.2lf %5.3lf\n", P_limitG,0.0,binpenConG,roLenConG,timeFactorG,0.0,0.0,ratioG);
	M( MSG, NULL, YmsgG ) ;
	fflush(fpoG);
}

int compute_attprcel(int flag)
{
	int n;
	...
	*/
	...
	if( moveable_cellsG <= 500 ) {
		...
		/*  n to the 4/3 power  */
		n = (int)(25.0 * pow((double) moveable_cellsG / 500.0, 1.0 / 3.0 )) ;
	}
...
int refine_fixed_placement()
{
	...
	if( prop_rigid_cellsG ) {
		...
		for( block = 1 ; block <= numRowsG ; block++ ) {
			start = carrayG[ pairArrayG[block][1] ]->cxcenter + carrayG[ pairArrayG[block][1] ]->tileptr->left ;
			end = carrayG[ pairArrayG[block][ pairArrayG[block][0] ] ]->cxcenter + carrayG[ pairArrayG[block][ pairArrayG[block][0] ] ]->tileptr->right ;
	...
	for( row = 1 ; row <= numRowsG ; row++ ) {
			...
			if( cellptr->cclass < 0 ) {  /* was "== -5" */
				if( cellptr->border >= 0 ) {
					if( prop_rigid_cellsG ) {
						...
						target = cellptr->border + barrayG[row]->bxcenter + barrayG[row]->bleft ;
					}
		...
		Yquicksort( (char *) ( rowptr + 1 ) , rowptr[0] , sizeof( int ), comparex ) ;
		Yquicksort( (char *) ( moveable_list + 1 ) , moveable_list[0] , sizeof( int ), comparex ) ;
			...
			*/
			if( save_m == -1 ) {  /* none fit */
				if( next_index[row] != -1 ) {
					...
					filled_to[row] = cellptr->cxcenter + cellptr->tileptr->right ;
					last_cell = p_array[row][0] ;
					i = ++next_index[row] ;
					while( i <= last_cell ) {
						cellptr = carrayG[ p_array[row][i] ] ;
				...
				if( last_index > 1 ) {
					...
					*/
					shift = carrayG[prev_cell]->cxcenter + carrayG[prev_cell]->tileptr->right - cellptr->tileptr->left - cellptr->cxcenter ;
					cellptr->cxcenter += shift ;
					/* ...update pin positions now..... */
					for( termptr = cellptr->pins; termptr; termptr = termptr->nextpin ) {
						termptr->xpos += shift ;
					...
					*/
					shift = barrayG[row]->bxcenter + barrayG[row]->bleft - cellptr->tileptr->left - cellptr->cxcenter ;
					cellptr->cxcenter += shift ;
					/* ...update pin positions now..... */
					for( termptr = cellptr->pins; termptr; termptr = termptr->nextpin ) {
						termptr->xpos += shift ;
