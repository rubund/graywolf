	...
	*/
	for( cell = 1 ; cell <= numcellsG - extra_cellsG ; cell++ ) {
		...
		if( cellptr->cycenter > 0 ) {
			carrayG[cell]->cxcenter = barrayG[blk]->bxcenter + barrayG[blk]->bleft + cellptr->cxcenter + widthS / 2 ;
			carrayG[cell]->cycenter = barrayG[blk]->bycenter ;
		} else {
			carrayG[cell]->cxcenter = barrayG[blk]->bxcenter + barrayG[blk]->bright + cellptr->cxcenter - (widthS - widthS / 2) ;
			carrayG[cell]->cycenter = barrayG[blk]->bycenter ;
		}
