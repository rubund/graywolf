...
void readblck(FILE *fp)
{
	...
	while( fscanf( fp , " %s " , input ) == 1 ) {
		...
		if( strcmp( input , "block") == 0 ) {
			...
			test = fscanf(fp, INTSCANSTR " " INTSCANSTR " " INTSCANSTR " " INTSCANSTR, &lowerL_x, &lowerL_y, &upperR_x, &upperR_y ) ;
			if( test != 4 ) {
	...
	exceptionsG = (EXCEPTBOX *) Ysafe_malloc( (1 + num_exceptsG) * sizeof( EXCEPTBOX ) ) ;
	...
	while( fscanf( fp , " %s " , input ) == 1 ) {
		...
		if( strcmp( input , "block" ) == 0 ) {
			...
			block++ ;
			fscanf(fp, INTSCANSTR " " INTSCANSTR " " INTSCANSTR " " INTSCANSTR, &lowerL_x, &lowerL_y, &upperR_x, &upperR_y ) ;
	...
	if( rowsG > 0 ) {  /*  then compute rowSepG */
		if( rowsG > 1 ) {
			...
			for( row = 2 ; row <= rowsG ; row++ ) {
				avg_row_sep += (double) barrayG[row]->bycenter - (double) barrayG[row-1]->bycenter ;
			}
