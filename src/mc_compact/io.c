...
void init( int numtiles, int numcells )
{
	...
	*/
	cellarrayG = (CELLBOXPTR *) Ysafe_malloc( (last_cellG+1)*sizeof(CELLBOXPTR) ) ;
	slackG = (CELLBOXPTR *) Ysafe_malloc( (numcellsG+1)*sizeof(CELLBOXPTR) ) ;
	for( i = 0; i <= numcellsG; i++ ){
		slackG[i] = cellarrayG[i] = (CELLBOXPTR) Ysafe_calloc( 1, sizeof(CELLBOX)) ;
	}
	...
	*/
	...
	for( i=1;i<=numcellsG;i++ ){
		...
		for( aptr = cptr->tiles;aptr; aptr = aptr->next ){
			ASSERTNCONT( aptr->node > 0 && aptr->node <= numtilesG, "preprocess_multi", "tile out of bounds\n" ) ;
			if( ++count >= 2 ){
				multi = TRUE ;
				break ;
			}
		...
		for( aptr = cptr->tiles;aptr; aptr = aptr->next ){
			...
			if( tptr->l_rel < minx || (tptr->l_rel == minx && aptr->node < cptr->xlo)){
				minx = tptr->l_rel ;
				cptr->xlo = aptr->node ;
			}
			if( tptr->b_rel < miny || (tptr->b_rel == miny && aptr->node < cptr->ylo)){
				miny = tptr->b_rel ;
				cptr->ylo = aptr->node ;
			}
			if( tptr->l_rel > maxx || (tptr->l_rel == maxx && aptr->node > cptr->xhi)){
				maxx = tptr->l_rel ;
				cptr->xhi = aptr->node ;
			}
			if( tptr->b_rel > maxy || (tptr->b_rel == maxy && aptr->node > cptr->yhi)){
				maxy = tptr->b_rel ;
				cptr->yhi = aptr->node ;
			}
		} /* end loop on tiles */
...
void init_extra_tile( int cell, int type )
{
	...
	numtilesG++ ;
	last_tileG = numtilesG + 1 ;
	tileNodeG = (COMPACTPTR *) Ysafe_realloc( tileNodeG, (numtilesG+1) * sizeof(COMPACTPTR) ); 
...
void process_tiles()
{
	...
	for( i = 1; i <= numtilesG; i++ ){
		t = tileNodeG[i] ;
		ASSERT( t->cell > 0 && t->cell <= numcellsG, "process_tiles", "cell out of bounds" ) ;
