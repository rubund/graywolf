...
void init_graph( int numnodes, int numedges )
{
	...
	changraphG = (CHANBOXPTR *) Ysafe_malloc( (numnodes+1)*sizeof(CHANBOXPTR) );
	for( i = 0; i <= numnodes; i++ ){
		changraphG[i] = (CHANBOXPTR) Ysafe_malloc( sizeof(CHANBOX) ) ;
		changraphG[i]->adj = NULL ;
	}
	edgeArrayG = (INFOPTR *) Ysafe_malloc( (numedges+1)*sizeof(INFOBOX) );
...
{
	CHANBOXPTR nptr ;
	ASSERTNRETURN( 1 <= node && node <= numnodesG, "addNode", "node out of range" ) ;
...
void addEdge( int node1 , int node2, BOOL HnotV, int cell_lb, int cell_rt ) 
{
	...
	/* cell on left (bottom) for vert (horz) channel */
	/* cell on right (top) for  vert (horz) channel */
...
ADJPTR findAdjPtr( int node1, int node2 )
{
	ADJPTR curEdge ;
