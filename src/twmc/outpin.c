...
static int output_pin( PINBOXPTR pinptr );
...
static int process_analog_net( NETBOXPTR netptr )
{
	int i, j ;           /* counters */
	...
	for( i = 0; i < aptr->num_common_points; i++ ) {
		...
		for( j = 0; j <= howmany; j++ ) {
			...
			ASSERTNCONT( pin > 0 && pin < numpinsG, "outpin", "Trouble with common point\n" ) ;
		...
		if( cpt->res_match ){
			...
			for( j = 1; j <= howmany; j++ ){
				pin = cpt->res_match[j] ;
				ASSERTNCONT( pin > 0 && pin < numpinsG, "outpin", "Trouble with res_match\n" );
				pinname = termarrayG[pin]->pinname ; 
				len += strlen( pinname ) ;
				if( j > 1 ){
					fprintf( fpS, ", " ) ;
